...
class DataSyncService:
    # Pass game_service and db_manager instances
    ...
    async def stop(self):
        ...
        if self._sync_task:
            ...
            except asyncio.CancelledError: logger.info("Data sync task cancelled.")
            except asyncio.TimeoutError: logger.warning("Data sync task did not cancel within timeout.")
            except Exception as e: logger.error(f"Error awaiting data sync task cancellation: {e}")
            finally: self._sync_task = None
    ...
    async def _daily_sync_loop(self):
        ...
        while self.running:
            next_run_time = None # Define outside try block
            try:
                ...
                # Schedule for 3 AM UTC the *next* day
                next_run_time = (now + timedelta(days=1)).replace(hour=3, minute=0, second=0, microsecond=0)
                # If it happens to be *exactly* 3 AM, this still schedules for the next day, which is fine.
    ...
    async def _sync_all_data(self):
        ...
        try:
            ...
            for sport in all_sports:
                 if not API_HOSTS.get(sport): continue # Skip if host not configured
                 try:
                      sport_leagues = await self._sync_leagues(sport)
                      if sport_leagues: synced_leagues.extend(sport_leagues)
                      await asyncio.sleep(1.1) # Avoid hitting rate limits (adjust delay as needed)
                 except Exception as e: logger.error(f"Error syncing leagues for sport '{sport}': {e}")

            # Sync Teams
            if synced_leagues: await self._sync_teams(synced_leagues)
            else: logger.warning("Skipping team sync as no leagues were synced.")
            ...
            else: logger.warning("Skipping schedule sync as no leagues were synced.")
            await asyncio.sleep(1.1)
            ...
            else: logger.warning("Skipping standings sync as no leagues were synced.")

            logger.info("Core data sync finished.")
        except Exception as e: logger.exception(f"Error during _sync_all_data: {e}")
    ...
    async def _sync_leagues(self, sport: str) -> List[Dict]:
        ...
        try:
            ...
            # Upsert using %s and MySQL ON DUPLICATE KEY UPDATE
            if processed_leagues:
                # Use VALUES() function in MySQL update clause
                upsert_query = """
                     ...
                     ON DUPLICATE KEY UPDATE
                         ...
                         sport = VALUES(sport)
                     """
                # Prepare data for executemany if db_manager supports it, otherwise loop
                data_tuples = [
                    (lg['id'], lg['name'], lg['type'], lg['logo'], lg['country'], lg['country_code'],
                     lg['country_flag'], lg['season'], lg['sport'])
                    for lg in processed_leagues
                ]
                # Looping execute for simplicity if executemany isn't available/easy in db_manager
                count = 0
        ...
        except Exception as e: logger.exception(f"Error syncing leagues for {sport}: {e}"); return []
    ...
    async def _sync_teams(self, leagues: List[Dict]):
        ...
        for league in leagues:
            ...
            if sport: leagues_by_sport.setdefault(sport, []).append(league)
                ...
                try:
                    ...
                    for team_entry in teams_api:
                        ...
                        processed_teams.append({
                            ...
                            'venue_id': venue.get('id'), # Added venue_id
                    ...
                    # Upsert teams using %s and MySQL ON DUPLICATE KEY UPDATE
                    if processed_teams:
                         upsert_query = """
                         ...
                         for data_tuple in data_tuples:
                             try:
                                 await self.db.execute(upsert_query, *data_tuple)
                                 count += 1
                             except Exception as e: logger.error(f"Error upserting team {data_tuple[0]}: {e}")

                         all_processed_teams.extend(processed_teams)
                         logger.debug(f"Upserted {count}/{len(processed_teams)} teams for league {league_id}")
                    await asyncio.sleep(1.1) # Delay between leagues
                except Exception as e: logger.exception(f"Error syncing teams for league {league_id} ({sport}): {e}")
    ...
    async def _sync_schedules(self, leagues: List[Dict], days_ahead: int):
        ...
        start_date = datetime.now(timezone.utc); end_date = start_date + timedelta(days=days_ahead)
        leagues_by_sport: Dict[str, List[Dict]] = {}
        for league in leagues:
            sport = league.get('sport')
            if sport: leagues_by_sport.setdefault(sport, []).append(league)
        ...
        for sport, sport_leagues in leagues_by_sport.items():
            ...
            for league in sport_leagues:
                ...
                try:
                    ...
                    # Use the existing _upsert_games_from_api in game_service which now uses %s and ON DUPLICATE KEY UPDATE
                    await self.game_service._upsert_games_from_api(schedule_api, sport)
                    all_processed_games.extend(schedule_api) # Count based on API response length

                    await asyncio.sleep(1.1) # Delay
                except Exception as e: logger.exception(f"Error syncing schedule for league {league_id} ({sport}): {e}")
                   ...
                   try:
                        ...
                        for standing_group_entry in standings_api:
                             ...
                             for team_standing in standings_table:
                                 ...
                                 goals_stats = all_stats.get('goals', {}) # Renamed from 'goals' in schema
                                 normalized = {
                                     'league_id': league_id, 'team_id': team['id'], 'season': season, # Added season
                                     ...
                                     'status': team_standing.get('status'), 'description': team_standing.get('description'), # Added status/desc
                                     'group_name': team_standing.get('group'), # Use 'group' from API
                        ...
                        # Upsert standings using %s and MySQL ON DUPLICATE KEY UPDATE
                        if processed_standings:
                             # Ensure rank column name is quoted if it's a reserved keyword (`rank`)
                             upsert_query = """
                             ...
                             for data_tuple in data_tuples:
                                  try:
                                       await self.db.execute(upsert_query, *data_tuple)
                                       count += 1
                                  except Exception as e: logger.error(f"Error upserting standing for team {data_tuple[1]} in league {data_tuple[0]}: {e}")
                        ...
                        await asyncio.sleep(1.1) # Delay
                   except Exception as e: logger.exception(f"Error syncing standings for league {league_id} ({sport}): {e}")
