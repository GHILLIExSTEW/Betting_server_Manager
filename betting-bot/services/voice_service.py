...
class VoiceService:
    ...
    async def get_voice_channel(self, channel_id: int) -> Optional[Dict[str, Any]]:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM voice_channels WHERE id = ?', (channel_id,)) as cursor:
                row = await cursor.fetchone()
                if row:
                    return dict(row)
                return None

    async def create_voice_channel(self, guild_id: int, name: str) -> int:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                'INSERT INTO voice_channels (guild_id, name, created_at) VALUES (?, ?, datetime("now"))',
                (guild_id, name)
            )
            await db.commit()
